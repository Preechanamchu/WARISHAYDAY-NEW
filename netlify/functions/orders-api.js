  ...
  try {
    ...
    while (attempts < maxAttempts && !success) {
      try {
        ...
        } else if (err.code === '42703' && err.message.includes('upgrade_snapshot')) {
          // กรณีคอลัมน์ upgrade_snapshot ยังไม่มีในฐานข้อมูล (Auto-Migration)
          console.warn(`Column upgrade_snapshot missing. Attempting to add column...`);
          try {
            await client.query('ALTER TABLE orders ADD COLUMN upgrade_snapshot TEXT');
            console.log('Column upgrade_snapshot added successfully.');
  ...
  try {
    ...
    if (currentStatus === 'new' && status === 'active') {
      for (const productId in items) {
        ...
        // ===== FIX: Skip virtual packages (coin_01, diamond_01, voucher_01, etc.) =====
        // Virtual product IDs are non-numeric strings — parseInt returns NaN which breaks PostgreSQL
        const numericId = parseInt(productId);
        if (!isFinite(numericId)) continue; // Skip non-numeric IDs (virtual packages)
        // ===== END FIX =====
        if (quantity > 0) {
          ...
          `;
          await client.query(updateStockQuery, [quantity, numericId]);
        }
      }
    }
